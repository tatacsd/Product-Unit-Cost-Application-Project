...
public class ProductUnitCostController {
	...
	@GetMapping("/variablecosts")
		public ResponseEntity<List<VariableCosts>> getAllVariableCosts(){
			try {
				List<VariableCosts> variableCostsList = new ArrayList<VariableCosts>();
				variableCostsRepository.findAll().forEach(variableCostsList::add);
				return new ResponseEntity<>(variableCostsList, HttpStatus.OK);
